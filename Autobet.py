...
class AutoBet:
    ...
    def read_conditions(self):
        ...
        self.skip_cnt_2nd_stage = int(myXMLtree.find('SkipCount2').text)
    ...
    def exist_condition(self, _g_title, _key):
        ...
        for i in range(cnt):
            ...
            if qq in self.condition_list[_key]:
                continue
            if (qq==0 or qq==-1) and self.flag_zero_include:
                continue
            flag_found = False
            break
        if flag_found:
    ...
    def play_roulette(self, _g_title, _cur_key):
        ...
        while True:
            if bet_now:
                ...
                if not _second_bet:
                    bet_amount = self.calc_normal_bet_amount_1st(
                        _g_title, stage)
                else:
                    bet_amount = self.calc_normal_bet_amount_2nd(
                        _g_title, stage)
                ...
                if not _second_bet:
                    zero_bet_amount = self.calc_zero_bet_amount(
                        _g_title, stage)
                else:
            ...
            if _second_bet and _second_check < self.skip_cnt_2nd_stage and not bet_now:
                if not new_num in self.condition_list[_cur_key]:
                    break
                _second_check += 1
                if _second_check == self.skip_cnt_2nd_stage:
                    bet_now = True
                continue
    ...
    def startProcess(self):
        ...
        while True:
            ...
            for i in range(ltcnt):
                ...
                print('\tCurrent number:      '+ self.change_color_text([numbers[0]]))
                
                print('\tHistory   list:      '+ self.change_color_text(self.games[_g_title]))
                
                # ---------------------------------------
                print(f"\n\t    👀 found repetition : " +   '\033[93m' + f"{cur_cdt} - {self.conditions[_g_title][cur_cdt]}" + "\033[0m")
